...
export default function NotificationsPage() {
  ...
  return (
    <div className="max-w-lg mx-auto">
      {/* Header */}
      <div className="sticky top-[var(--nav-h)] bg-[var(--bg-base)] px-4 py-3 border-b border-[var(--border)] flex items-center justify-between z-10">
        <h1 className="text-lg font-bold text-[var(--text-primary)]">
          Notificaciones
          {unread.length > 0 && (
            <span className="ml-2 text-sm font-normal text-[var(--text-muted)]">({unread.length})</span>
          )}
        </h1>
        {unread.length > 0 && (
          <button
      ...
      {loading ? (
        ...
        <>
          {newNotifs.length > 0 && (
            <>
              <div className="px-4 py-2 bg-[var(--bg-elevated)]">
                <span className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider">Nuevas</span>
              </div>
              <div className="divide-y divide-[var(--border)]">
          ...
          {oldNotifs.length > 0 && (
            <>
              <div className="px-4 py-2 bg-[var(--bg-elevated)]">
                <span className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider">Anteriores</span>
              </div>
              <div className="divide-y divide-[var(--border)]">
